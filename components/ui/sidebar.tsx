...
// Context for individual sidebar toggle (used by Sidebar component)
type SidebarItemContextProps = {
  toggle: () => void
}

const SidebarItemContext = React.createContext<SidebarItemContextProps | null>(null)

function useSidebarToggle() {
  const context = React.useContext(SidebarItemContext)
  if (!context) {
    throw new Error("useSidebarToggle must be used within a Sidebar.")
  }
  return context
}
...
function Sidebar({
  ...
  const itemContextValue = React.useMemo<SidebarItemContextProps>(
    () => ({ toggle }),
    [toggle]
  )
